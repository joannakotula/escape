 ...
 ************************************************/
 ...
 (function ($) {
	$.fn.qrcode = function (options) {
		var settings = $.extend({
			urlBase: new URL("../puzzles.html", document.baseURI).href,
			url: null,
			showUrl: true,
			marker: null,
			innerClass: 'qr_inner',
		...
		var getUrl = function(el) {
			...
			if (elementUrl) {
				return settings.urlBase + elementUrl;
			}
			return settings.url;
		}
		var createInner = function(size) {
			var inner = $(`<div class='${settings.innerClass}'></div>`);
			inner.width(size);
			return inner;
		}
		...
		var createMarker = function(){
			var marker = $(`<img src='${settings.marker}' class=${settings.markerClass} />`);
			return marker;
		}

		var createUrlDiv = function(url){
			return $(`<div class='${settings.urlClass}'>${url}</div>`)
		}
		...
		return this.each(function() {
			...
			if (settings.marker) {
				inner.append(createMarker());
			}
			if (settings.showUrl) {
				element.append(createUrlDiv(url));
			}
		});  
	}
 }(jQuery));